...
$pageSeo = function_exists('get_academic_page_info') ? get_academic_page_info('HEIHandbook') : [];
$page_data = $pageSeo;
$meta_title = !empty($pageSeo['meta_title']) ? $pageSeo['meta_title'] : 'HEI Handbook - SSSUTMS';
...
$banner_title = $pageSeo['banner_title'] ?? ($pageSeo['page_title'] ?? 'HEI Handbook');
$banner_category = $pageSeo['banner_category'] ?? 'Academic';
$academicDocs = function_exists('get_academic_documents') ? get_academic_documents('HEIHandbook') : [];
          ...
          <div class="p-4">
            ...
            <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px; padding:20px; margin-top:24px;">
              <h5 style="color:#0b2545; font-weight:700; font-size:1.05rem; margin-bottom:14px; display:flex; align-items:center; gap:8px;">
                <i class="fa fa-folder-open text-warning"></i> Additional HEI Handbooks, Acts &amp; Codes
              </h5>
              <div class="row g-2">
                <?php foreach ($academicDocs as $doc): 
                  $dTitle = !empty($doc['title']) ? $doc['title'] : 'HEI Document';
