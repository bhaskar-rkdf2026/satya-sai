...
foreach ($files as $fpath) {
    echo "Updating hover & alignment in $fpath...\n";
    $content = file_get_contents($fpath);

    // Convert <tr> with <td><strong>... Header text to proper <thead><th>
    $content = preg_replace_callback('/<tr[^>]*>\s*<td[^>]*>\s*<strong>\s*(S\.No\.|School Name|Facilities|Sr\.No\.|Criteria)[^<]*<\/strong>\s*<\/td>.*?<\/tr>/is', function($matches) {
        $trHtml = $matches[0];
        // Replace td with th
        $trHtml = str_replace('<td', '<th', $trHtml);
        $trHtml = str_replace('</td', '</th', $trHtml);
        return '<tr class="naac-table-header">' . substr($trHtml, strpos($trHtml, '>') + 1);
    }, $content);

    // CSS system for perfect alignment, header contrast, and smooth hover
    $css_system = <<<CSS
<style>
.naac-section { 
...
.naac-custom-table td {
  ...
  text-align: left !important; /* Uniform left alignment for all data columns */
}
.naac-custom-table td * {
  color: #334155;
}
...
CSS;
    ...
    $content = preg_replace('/<style>.*?<\/style>/is', $css_system, $content);
    file_put_contents($fpath, $content);
    echo "  [DONE] Updated $fpath\n";
}

echo "All Criteria pages updated with uniform alignment, high-contrast headers, and smooth hover!\n";
