...
foreach ($files as $fpath) {
    echo "Refining typography and metrics in $fpath...\n";
    $content = file_get_contents($fpath);

    // 1. Fix common spelling errors
    $content = str_replace('Curriculam', 'Curriculum', $content);
    $content = str_replace('Programme outcomes', 'Programme Outcomes', $content);
    $content = str_replace('Course Outcomes(COs)', 'Course Outcomes (COs)', $content);
    $content = str_replace('Course Outcomes(COs', 'Course Outcomes (COs', $content);

    // 2. Merge split metric paragraph blocks
    // Pattern matches <p>1.1.1 ...</p> <p> ...</p> <p> ...</p>
    $content = preg_replace_callback('/<p>\s*(\d+\.\d+(?:\.\d+)?(?:[a-z])?)\s+(.*?)<\/p>(?:\s*<p>(.*?)<\/p>)*/is', function($matches) {
        $fullMatch = $matches[0];
        $metricCode = $matches[1];

        // Extract all text inside paragraphs for this metric block
        preg_match_all('/<p>(.*?)<\/p>/is', $fullMatch, $pmatch);
        $fullText = implode(' ', $pmatch[1]);
        $fullText = preg_replace('/^' . preg_quote($metricCode, '/') . '\s*/i', '', $fullText);
        $fullText = preg_replace('/&nbsp;/', ' ', $fullText);
        $fullText = preg_replace('/\s+/', ' ', trim($fullText));

        // Strip inline style tags inside text
        $fullText = preg_replace('/<span[^>]*>/i', '', $fullText);
        $fullText = str_replace('</span>', '', $fullText);
...
<div class="naac-metric-box">
  <div class="d-flex align-items-start gap-3">
    <span class="naac-metric-badge">Metric {$metricCode}</span>
...
HTML;
    ...
    // 3. Inject CSS for sharp readability & metric callout boxes
    $css_typography = <<<CSS
<style>
.naac-section { 
...
/* Table Header Row Styling - High Contrast Crisp White on Dark Navy */
...
CSS;
    ...
    $content = preg_replace('/<style>.*?<\/style>/is', $css_typography, $content);
    file_put_contents($fpath, $content);
    echo "  [DONE] Refined $fpath\n";
}

echo "All Criteria pages updated with crisp typography and metric callout boxes!\n";
