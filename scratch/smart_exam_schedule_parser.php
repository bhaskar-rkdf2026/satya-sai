...
foreach ($local_files as $lf) {
    $file_map[strtolower(basename($lf))] = '<?php echo BASE_URL; ?>assets/images/Files/Link/ExamSchedules/' . rawurlencode(basename($lf));
}

// Also check root Link dir
$link_files = glob('d:/xampp/htdocs/sssu/satya-sai/assets/images/Files/Link/*.pdf');
foreach ($link_files as $lf) {
    $file_map[strtolower(basename($lf))] = '<?php echo BASE_URL; ?>assets/images/Files/Link/' . rawurlencode(basename($lf));
}

echo "Total indexed PDFs: " . count($file_map) . "\n";
...
foreach ($tables as $tIdx => $table) {
    ...
    for ($i = 0; $i < $rows->length; $i++) {
        ...
        // Iterate anchors to pick the most specific text anchor and matching pdf
        foreach ($anchors as $a) {
            $href = trim($a->getAttribute('href'));
            $atxt = clean_text($a->textContent);
            $img = $xpath->query('.//img', $a);

            // Skip anchor if it's purely an icon link with no text content
            if ($img->length > 0 && empty($atxt)) {
                if (empty($bestUrl) && !empty($href) && strpos($href, '.pdf') !== false) {
                    $bestUrl = $href;
                }
                continue;
            }
            ...
            if (!empty($href) && $href !== '#' && strpos($href, 'javascript') === false) {
                if (!empty($atxt) && strlen($atxt) > 3) {
                    $bestTitle = $atxt;
                    if (strpos($href, '.pdf') !== false || empty($bestUrl)) {
                        $bestUrl = $href;
                    }
                }
            }
        ...
        // Clean title
        $bestTitle = preg_replace('/^(Link|Click here|Download|S\.No\.\s*\d+)/i', '', $bestTitle);
        $bestTitle = preg_replace('/^\d+\s+/', '', $bestTitle);
        $bestTitle = trim($bestTitle);

        if (!empty($bestTitle) && strlen($bestTitle) > 2) {
            // Resolve PDF link accurately
            $finalLink = '#';
            if (!empty($bestUrl)) {
                $fname = strtolower(basename(urldecode(parse_url($bestUrl, PHP_URL_PATH))));
                if (isset($file_map[$fname])) {
                    $finalLink = $file_map[$fname];
                } else {
                    // Try to match PDF filename against title keywords
                    foreach ($file_map as $bn => $link) {
                        if (strpos($bn, $fname) !== false || (!empty($fname) && strlen($fname) > 5 && strpos($bn, substr($fname, 0, 10)) !== false)) {
                            $finalLink = $link;
                            break;
                        }
                    }
            ...
            // Fallback: If still '#', search file_map for course name keywords in title
            if ($finalLink === '#') {
                $cleanTitleLower = strtolower($bestTitle);
                foreach ($file_map as $bn => $link) {
                    if (preg_match('/bams/i', $cleanTitleLower) && strpos($bn, 'bams') !== false) {
                        $finalLink = $link; break;
                    } elseif (preg_match('/bhms/i', $cleanTitleLower) && strpos($bn, 'bhms') !== false) {
                        $finalLink = $link; break;
                    } elseif (preg_match('/b\.sc\s*\(hons\)/i', $cleanTitleLower) && (strpos($bn, 'bsc') !== false || strpos($bn, 'ag') !== false)) {
                        $finalLink = $link; break;
                    } elseif (preg_match('/bca/i', $cleanTitleLower) && strpos($bn, 'bca') !== false) {
                        $finalLink = $link; break;
                    } elseif (preg_match('/bba/i', $cleanTitleLower) && strpos($bn, 'bba') !== false) {
                        $finalLink = $link; break;
                    } elseif (preg_match('/b\.e\.|bachelor of engineering/i', $cleanTitleLower) && strpos($bn, 'be') !== false) {
                        $finalLink = $link; break;
                    } elseif (preg_match('/mca/i', $cleanTitleLower) && strpos($bn, 'mca') !== false) {
                        $finalLink = $link; break;
                    } elseif (preg_match('/mba/i', $cleanTitleLower) && strpos($bn, 'mba') !== false) {
                        $finalLink = $link; break;
                    } elseif (preg_match('/ph\.d|phd/i', $cleanTitleLower) && (strpos($bn, 'phd') !== false || strpos($bn, 'docscanner') !== false)) {
                        $finalLink = $link; break;
                    }
                }
            }
...
echo "Extracted " . count($sections) . " sections with ACCURATE PDF MAPPING!\n";
...
echo "Examination/ExamSchedule.php successfully rebuilt with ACCURATE Course-to-PDF mappings!\n";

PHP;

file_put_contents('scratch/smart_exam_schedule_parser.php', $code);
