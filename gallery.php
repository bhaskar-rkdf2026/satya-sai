...
$page_info = get_gallery_page_info();
$categories = get_gallery_categories(true);
$all_photos = get_gallery_photos('all', true);
$videos = get_gallery_videos(true);

$page_data = $page_info;
$page_title = $page_info['meta_title'] ?? ($page_info['page_title'] ?? 'Photo & Video Gallery - SSSUTMS');
$banner_title = $page_info['banner_title'] ?? 'Campus Photo & Video Gallery';
$banner_category = $page_info['banner_category'] ?? 'Campus Life';
    ...
    <div class="syl-card">
      <div class="syl-card-header d-flex flex-wrap align-items-center justify-content-between gap-3">
        <h2 class="syl-card-title">
          ...
          <?php echo htmlspecialchars($page_info['heading'] ?? 'Campus Moments, Infrastructure & Life at SSSUTMS'); ?>
        </h2>
      ...
      <div class="syl-card-body">
        ...
        <div class="mb-4 text-center">
          ...
          <?php foreach ($categories as $catKey => $catData): ?>
            ...
            ?>
            <button type="button" class="gallery-filter-btn" onclick="setCategory('<?php echo htmlspecialchars($catKey); ?>', this)">
              <i class="fa <?php echo htmlspecialchars($catData['icon'] ?? 'fa-images'); ?>"></i> <?php echo htmlspecialchars($catData['name']); ?> (<?php echo $count; ?>)
            </button>
          <?php endforeach; ?>
        </div>
        ...
        <div class="row g-3 g-md-4" id="galleryGrid">
          <?php foreach ($all_photos as $index => $item): ?>
            <div class="col-xl-3 col-lg-4 col-md-6 photo-item-col" data-cat="<?php echo htmlspecialchars($item['cat']); ?>">
              <div class="photo-card">
                <a href="<?php echo htmlspecialchars($item['url']); ?>" target="_blank" rel="noopener" class="d-block w-100 h-100">
                  <img src="<?php echo htmlspecialchars($item['url']); ?>" loading="lazy" decoding="async" alt="<?php echo htmlspecialchars($item['title']); ?>">
                </a>
                <div class="photo-overlay">
                  <div>
                    <span class="badge bg-warning text-dark mb-1" style="font-size: 10px;"><?php echo htmlspecialchars($item['badge']); ?></span>
                  </div>
        ...
        <?php if (!empty($videos)): ?>
          <div class="department-heading">
            <i class="fa fa-film"></i> <?php echo htmlspecialchars($page_info['video_heading'] ?? 'Campus Video Tour & Convocation Highlights'); ?>
          </div>
              ...
              <div class="col-md-6 d-flex">
                <div class="border rounded-3 overflow-hidden bg-white shadow-sm d-flex flex-column w-100 h-100">
                  <div class="ratio ratio-16x9 bg-dark flex-shrink-0">
                    <iframe src="<?php echo htmlspecialchars($vid['embed_url']); ?>" title="<?php echo htmlspecialchars($vid['title']); ?>" allowfullscreen loading="lazy"></iframe>
                  </div>
                  <div class="p-3 bg-white border-top d-flex flex-column justify-content-between flex-grow-1">
                    <h6 class="fw-bold text-dark mb-2"><?php echo htmlspecialchars($vid['title']); ?></h6>
                    <p class="text-secondary small mb-0 flex-grow-1"><?php echo htmlspecialchars($vid['description']); ?></p>
                  </div>
                </div>
              </div>
            <?php endforeach; ?>
