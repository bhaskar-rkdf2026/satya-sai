<?php
require_once __DIR__ . '/../config.php';
require_admin_auth();

$events = get_events();
$msg = '';

// Handle Delete
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    $delId = (int)$_GET['id'];
    $events = array_filter($events, function($e) use ($delId) {
        return ($e['id'] ?? 0) !== $delId;
    });
    save_json_data('events.json', array_values($events));
    $msg = 'Event removed successfully.';
}

// Handle Add Event
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_event') {
    $title = clean_input($_POST['title'] ?? '');
    $category = clean_input($_POST['category'] ?? 'General');
    $date = clean_input($_POST['date'] ?? date('Y-m-d'));
    $desc = clean_input($_POST['description'] ?? '');
    $loc = clean_input($_POST['location'] ?? 'SSSUTMS Campus');

    if (!empty($title)) {
        $newEvent = [
            'id' => time(),
            'title' => $title,
            'category' => $category,
            'date' => $date,
            'description' => $desc,
            'location' => $loc,
            'image' => 'assets/images/events/scienceday.jpg'
        ];
        array_unshift($events, $newEvent);
        save_json_data('events.json', $events);
        $msg = 'Event created & published successfully!';
    }
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Manage Events - SSSUTMS Admin</title>
  <link rel="icon" type="image/jpeg" href="../assets/images/logo/logo.jpg">
  <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
  <link rel="stylesheet" href="../assets/css/admin.css">
</head>
<body>

<aside class="admin-sidebar">
  <div class="sidebar-brand">
    <img src="../assets/images/logo/logo.jpg" alt="Logo" width="38" height="38" class="rounded-circle border">
    <div>
      <h6 class="text-white fw-bold mb-0">SSSUTMS Admin</h6>
      <small class="text-warning">Management Portal</small>
    </div>
  </div>

  <ul class="admin-nav">
    <li><a href="index.php" class="nav-link"><i class="fa fa-gauge"></i> Dashboard</a></li>
    <li><a href="notices.php" class="nav-link"><i class="fa fa-bullhorn"></i> Notices & Circulars</a></li>
    <li><a href="inquiries.php" class="nav-link"><i class="fa fa-envelope-open-text"></i> Admission Leads</a></li>
    <li><a href="applications.php" class="nav-link"><i class="fa fa-user-graduate"></i> Student Registrations</a></li>
    <li><a href="events.php" class="nav-link active"><i class="fa fa-calendar-days"></i> Events & Workshops</a></li>
    <li><a href="settings.php" class="nav-link"><i class="fa fa-sliders"></i> Portal Settings</a></li>
    <li class="mt-4 pt-3 border-top border-white border-opacity-10">
      <a href="../index.php" target="_blank" class="nav-link text-white-50"><i class="fa fa-arrow-up-right-from-square"></i> Visit Public Site</a>
    </li>
    <li><a href="logout.php" class="nav-link text-danger"><i class="fa fa-right-from-bracket"></i> Sign Out</a></li>
  </ul>
</aside>

<main class="admin-main">
  
  <header class="admin-topbar">
    <div>
      <h5 class="fw-bold text-dark mb-0">Campus Events & Workshops Manager</h5>
      <small class="text-muted">Manage conferences, FDPs, cultural festivals and placement drives</small>
    </div>
    <button class="btn btn-primary btn-sm fw-bold rounded-pill px-3" data-bs-toggle="modal" data-bs-target="#addEventModal" style="background: var(--admin-primary); border:none;">
      <i class="fa fa-plus me-1"></i> Add New Event
    </button>
  </header>

  <?php if (!empty($msg)): ?>
    <div class="alert alert-success alert-dismissible fade show rounded-3 border-0 shadow-sm" role="alert">
      <i class="fa fa-circle-check me-2"></i> <?php echo htmlspecialchars($msg); ?>
      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>
  <?php endif; ?>

  <div class="row g-4">
    <?php foreach ($events as $evt): ?>
      <div class="col-md-6 col-xl-4">
        <div class="card border-0 shadow-sm rounded-4 h-100 bg-white overflow-hidden">
          <div style="height: 160px; overflow:hidden;">
            <img src="../<?php echo htmlspecialchars($evt['image']); ?>" alt="Event" class="w-100 h-100 object-fit-cover" onerror="this.src='../assets/images/slider/aamh(2).jpg'">
          </div>
          <div class="card-body p-3 d-flex flex-column justify-content-between">
            <div>
              <span class="badge bg-primary-subtle text-primary fw-bold mb-2"><?php echo htmlspecialchars($evt['category']); ?></span>
              <h6 class="fw-bold text-dark mb-1"><?php echo htmlspecialchars($evt['title']); ?></h6>
              <p class="small text-muted mb-2"><?php echo htmlspecialchars($evt['description']); ?></p>
            </div>
            <div class="pt-2 border-top d-flex justify-content-between align-items-center">
              <small class="text-muted"><i class="fa fa-calendar me-1"></i> <?php echo date('d-M-Y', strtotime($evt['date'])); ?></small>
              <a href="events.php?action=delete&id=<?php echo $evt['id']; ?>" class="btn btn-outline-danger btn-sm rounded-circle" onclick="return confirm('Delete this event?');">
                <i class="fa fa-trash"></i>
              </a>
            </div>
          </div>
        </div>
      </div>
    <?php endforeach; ?>
  </div>

</main>

<!-- Add Event Modal -->
<div class="modal fade" id="addEventModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow-lg rounded-4">
      <div class="modal-header text-white" style="background: var(--admin-primary);">
        <h5 class="modal-title fw-bold">Add New Campus Event</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form method="POST" action="events.php">
        <input type="hidden" name="action" value="add_event">
        <div class="modal-body p-4">
          <div class="mb-3">
            <label class="form-label small fw-bold">Event Title *</label>
            <input type="text" name="title" class="form-control" placeholder="e.g. National Symposium on Health" required>
          </div>
          <div class="row g-3 mb-3">
            <div class="col-md-6">
              <label class="form-label small fw-bold">Category</label>
              <select name="category" class="form-select">
                <option value="Symposium">Symposium / Conference</option>
                <option value="FDP">Faculty Development Program</option>
                <option value="Placement">Mega Placement Drive</option>
                <option value="Workshop">Technical Workshop</option>
                <option value="Cultural">Cultural Festival</option>
              </select>
            </div>
            <div class="col-md-6">
              <label class="form-label small fw-bold">Event Date *</label>
              <input type="date" name="date" class="form-control" value="<?php echo date('Y-m-d'); ?>" required>
            </div>
          </div>
          <div class="mb-3">
            <label class="form-label small fw-bold">Event Location / Venue</label>
            <input type="text" name="location" class="form-control" placeholder="Main Auditorium, SSSUTMS">
          </div>
          <div class="mb-3">
            <label class="form-label small fw-bold">Event Summary Description</label>
            <textarea name="description" class="form-control" rows="3" placeholder="Details about speakers, eligibility, agenda..."></textarea>
          </div>
        </div>
        <div class="modal-footer bg-light">
          <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Cancel</button>
          <button type="submit" class="btn btn-primary btn-sm fw-bold px-4" style="background: var(--admin-primary); border:none;">Publish Event</button>
        </div>
      </form>
    </div>
  </div>
</div>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>
